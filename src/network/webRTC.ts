...
import {LiveLineEnum} from "../../types";
import {WsMsgTypeEnum} from "../../types/websocket";

// import {nanoid} from 'nanoid'

/**
 * 通过applyConstraints设置分辨率和帧率
 * @param data
 */
export async function handleConstraints(data: {
    ...
    const {frameRate, height, stream, width} = data;

    const queue: Promise<any>[] = [];
    console.log('开始设置帧率和分辨率', JSON.stringify({height, frameRate}));
    ...
    try {
        ...
        console.error('设置约束失败', error);
        return 0;
    }
}

/**
 * @description RTC实体类，可以实现RTC对等连接，多对多，一对多
 * 还可以实现推流RTC
 */
export class WebRTCClass {
    ...
    /**RTC信道，每个数据通道都与RTCPeerConnection相关联，每个RTC连接理论上最多可以有65534个数据通道*/
        ...
        this.peerConnection.addEventListener(
            ...
            (event: any) => {
                ...
                if (iceConnectionState === 'connected') {
                    ...
                    appStore.remoteDesk.isRemoting = true;
            ...
            (event: any) => {
                ...
                if (connectionState === 'connected') {
                    ...
                    if (this.maxBitrate !== -1) {
                        this.setMaxBitrate(this.maxBitrate).then(() => null);
                    }
                }
    ...
    close = () => {
        try {
            ...
            appStore.remoteDesk.isRemoting = false;
